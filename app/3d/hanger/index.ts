...
import { SocketEnum, getSocket } from "./sockets";
import { getWing } from "./wing";
import { getUShapeSlot } from "./u-shape-slot";
...
const getHanger = (params: HangerParams) => {
  ...
  const uShape = getUShapeSlot(hubBoundingBox[1][0], hubHeight);
  const uShapeBbox = measureAggregateBoundingBox(uShape);
  const mount = translate(
    [0, -hubBoundingBox[1][1], hubBoundingBox[1][2] + uShapeBbox[1][0]],
    rotate([0, TAU / 4, 0], uShape)
  );

  const hanger = union([mount, hub]);
